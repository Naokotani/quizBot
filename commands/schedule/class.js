...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addSubcommand(subcommand =>
			subcommand
				.setName('show')
				.setDescription('Show the class list'))
		.addSubcommand(subcommand =>
			subcommand
				.setName('status')
		...
		function add(type){
			if (type === 'reply') {
				...
				db.run("INSERT INTO classes (className, active) VALUES (?, ?)", {
					...
					err?console.error(err):console.log(className + " Successfully Created");
					interaction.editReply(err?"Failed to create " + className:className + " created successfully");
				});
			}
		}
